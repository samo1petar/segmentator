...
from lib.tools.TrainSupportCard import TrainSupport
...
def train(
    ...
    while iter < max_iter:
        ...
        if iter % eval_every_iter == 0:
            ...
            for path, cls_name, cls_id, image in loader.read_record('test'):
                prediction = model(image, training=False)
                loss = loss_object(cls_id, prediction)
                test_loss(loss)
            tf.summary.scalar('test_loss', test_loss.result(), iter)
            test_loss.reset_states()

        if iter % sample_every_iter == 0:
            ...
            model.save_weights(os.path.join(train_support.model_saving_dir, 'model_' + str(iter.numpy())),
                               save_format='tf')
        ...
        ######################  TRAIN STEP ######################
        path, cls_name, cls_id, image = iterator_train.__next__()
        start = time.time()
        train_step(image, cls_id)
        end = time.time()
