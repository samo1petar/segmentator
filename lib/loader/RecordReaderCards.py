    ...
    def read_record(self, set_name: str) -> Generator[tf.Tensor, None, None]:
        ...
        def parse(x):
            keys_to_features = {
                ...
                'cls_id'   : tf.io.FixedLenFeature([], tf.int64),
            ...
            return (
                parsed_features['path'],
                parsed_features['cls_name'],
                parsed_features['cls_id'],
        ...
        for path, cls_name, cls_id, image in iter(dataset):

            rot_cond = tf.random.uniform([], 0, 4, dtype=tf.int32)

            if set_name == 'train':

                image_2 = tf.image.random_brightness(image, 0.1)
                image_2 = tf.image.random_hue(image_2, 0.1)
                image_2 = tf.image.random_contrast(image_2, 0.0, 0.2)
                image_2 = tf.image.random_saturation(image_2, 2, 7)
